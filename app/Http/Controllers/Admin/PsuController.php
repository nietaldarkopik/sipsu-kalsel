...
class PsuController extends Controller
{
    ...
    {
        $kabupatenkotas = PsuModel::where('province_id',63)->orderBy('id', 'DESC')->paginate(20);
    ...
    public function store(Request $request)
    {
        ...
        $role = PsuModel::create(['name' => $request->input('name')]);
        $role->syncPermissions($request->input('permission'));

        return redirect()->route('admin.kabupatenkotas.index')
            ->with('success', 'Psu created successfully');
    }

    public function show($id)
    {
        $role = PsuModel::find($id);
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('admin.kabupatenkotas.index')
            ->with('success', 'Psu updated successfully');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('admin.kabupatenkotas.index')
            ->with('success', 'Psu deleted successfully');
    }
}
